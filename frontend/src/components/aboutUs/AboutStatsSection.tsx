      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 gap-6 sm:gap-8 md:grid-cols-4">
          {stats.map((stat, index) => (
            <div 
              key={index} 
              className="text-center space-y-3 sm:space-y-4 p-3 sm:p-4 rounded-xl hover:bg-gray-50 transition-colors duration-200"
            >
              ...
              <div className="space-y-1 sm:space-y-2">
                <div className="text-3xl xs:text-4xl sm:text-[2.5rem] md:text-5xl font-bold text-gray-900">
                  {stat.number}
                </div>
                <p className="text-gray-600 text-xs xs:text-sm sm:text-[0.9375rem] md:text-base font-medium leading-snug">
                  {stat.description}
                </p>
              </div>
            </div>
