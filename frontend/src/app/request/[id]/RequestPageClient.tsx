...
export function RequestPageClient() {
  ...
  const handleSubmitRequest = async () => {
    ...
    try {
      ...
      router.push(`/request/success?amount=${finalAmount}&requestId=${params.id}&donationId=${donationId}&transactionId=${requestId}`)
    } catch (err) {
      console.error('Request failed:', err)
      setVerificationError('Request failed. Please try again.')
    } finally {
    ...
    <MainLayout>
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="bg-card rounded-lg shadow-sm border p-6 mb-8">
          <h2 className="text-xl font-bold mb-4">Donation Info</h2>
          <p><span className="font-medium">Available amount:</span> {donationAmount.toLocaleString()}</p>
        ...
        <div className="bg-card rounded-lg shadow-sm border p-6">
          <h2 className="text-xl font-bold mb-6">Request the amount you need</h2>
